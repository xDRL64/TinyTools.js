...
const check_reqBodyData = (req, res)=>{
    ...
    if(dataType !== 'string'){
        ...
        if( !server_mime.includes(client_mime) ){
            msg += 'MIME type checking : API waiting for '
                +  `${server_mime.map(e=>`'${e}'`).join('|')} `
                +  `and received '${client_mime}' .`;
        }
        res.json({status:'failed', msg, data:null});
        return 'failed';
    }
...
// user confirmation for storage directory path
(()=>{
    let askAgain = true;
    const normal='\x1b[0m', yellow='\x1b[33m', green='\x1b[32m', red='\x1b[31m';
    while(askAgain){
        const info = `${normal}\nConfirme computed absolute Storage Path :\n`
                   + `${yellow}\t${storage.path}/\n`
                   + `${normal}Client app can create sessions as in the following example :\n`
                   + `${yellow}\t${storage.path}/session_name_exemple\n`
                   + `${normal}Do you confirm ?`;
        console.log(info);
        const question = `${green}y: yes ${normal}/ ${red}n: no ${normal}:`;
        const confirmation = prompt(question)?.toLowerCase() ?? 'n'; // ctrl+c returns 'n'
        const userResp = confirmation.match(/^(y|yes|n|no)$/)?.shift();
        if(userResp?.[0] === 'n') process.exit(1);
        askAgain = !userResp;
    }
})();
